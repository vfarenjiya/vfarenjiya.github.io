...
function parseMathText(text){
  ...
  text = text.replace(/\$\$([\s\S]*?)\$\$/g, (m,p1)=>{
    try{ return katex.renderToString(p1,{displayMode:true,throwOnError:false}); }catch(e){ return m; }
  });
  text = text.replace(/\$([^$\n]+?)\$/g, (m,p1)=>{
    try{ return katex.renderToString(p1,{displayMode:false,throwOnError:false}); }catch(e){ return m; }
  });
  return text;
}
...
/* FIXED: normalized MATH_DB lookup — trailing spaces can never break matching */
const MATH_DB = {};
...
function getMathematicalContent(t){
  ...
  if(MATH_DB[title]){
    ...
    if(t.s === 'ps'){
      ...
      ctr = '<span class="counter-title">Independent Axiom Collapse</span>Blindly using '+baseEquation+' when cross-covariance terms are non-zero creates compounding errors.';
      ...
      if(scenario===0){
        ex = '<p><strong>Advanced Problem ('+plainTopic+'):</strong> A randomized data pipeline relies on a system obeying '+baseEquation+'. Given a performance variable tracking parameter $\\alpha = '+var1+'$, the joint loss space bounds density uniformly over $f(x) = '+var1+'x^{'+(var1-1)+'}$ for $x \\in [0,1]$. Evaluate the exact system variance.</p>'+
             '<p><strong>Step-by-Step Solution:</strong><br>1. First raw moment: $E[X] = \\int_0^1 '+var1+'x^{'+var1+'}\\, dx = \\frac{'+var1+'}{'+(var1+1)+'}$.<br>2. Second raw moment: $E[X^2] = \\int_0^1 '+var1+'x^{'+(var1+1)+'}\\, dx = \\frac{'+var1+'}{'+(var1+2)+'}$.<br>3. Variance identity: $\\text{Var}(X) = \\frac{'+var1+'}{'+(var1+2)+'} - \\left(\\frac{'+var1+'}{'+(var1+1)+'}\\right)^2$.</p>';
      } else if(scenario===1){
        ex = '<p><strong>Advanced Problem ('+plainTopic+'):</strong> A protocol uses '+baseEquation+' to bound phase shifts. If the shift parameter is $\\lambda = '+var2+'$, evaluate $\\int_{0}^{\\infty} x^{'+var2+'} e^{-x}\\, dx$.</p>'+
             '<p><strong>Step-by-Step Solution:</strong><br>1. Gamma identity: $\\Gamma(n) = \\int_0^\\infty x^{n-1} e^{-x}\\, dx = (n-1)!$.<br>2. Match: $n-1 = '+var2+'$ so $n = '+(var2+1)+'$.<br>3. Exact value: $\\Gamma('+(var2+1)+') = '+var2+'!$.</p>';
      } else {
        ex = '<p><strong>Advanced Problem ('+plainTopic+'):</strong> A risk model relies on '+baseEquation+'. Default occurs if a shock $Z \\sim N(0,1)$ exceeds $z = '+(var1/2)+'$. Bound this tail using Markov on $Z^2$.</p>'+
             '<p><strong>Step-by-Step Solution:</strong><br>1. Apply Markov to $Z^2 \\sim \\chi^2_1$: $P(Z^2 \\ge '+Math.pow(var1/2,2)+') \\le \\frac{E[Z^2]}{'+Math.pow(var1/2,2)+'}$.<br>2. Since $E[Z^2]=1$, the risk ceiling is $\\frac{1}{'+Math.pow(var1/2,2)+'}$.</p>';
      }
    } else if(t.s === 'la'){
      ...
      if(scenario===0){
        ex = '<p><strong>Advanced Problem ('+plainTopic+'):</strong> A matrix has eigenvalues $\\lambda_1 = '+var1+'$ and $\\lambda_2 = '+var2+'$. Calculate $\\det(A^2)$.</p>'+
             '<p><strong>Step-by-Step Solution:</strong><br>1. $\\det(A) = \\lambda_1 \\lambda_2 = '+(var1*var2)+'$.<br>2. $\\det(A^2) = (\\det A)^2 = '+Math.pow(var1*var2,2)+'$.</p>';
      } else if(scenario===1){
        ex = '<p><strong>Advanced Problem ('+plainTopic+'):</strong> A scaling matrix $S$ has trace '+var3+'. Two eigenvalues equal '+var1+'. Find the third.</p>'+
             '<p><strong>Step-by-Step Solution:</strong><br>1. Trace = sum of eigenvalues: $'+var3+' = '+var1+' + '+var1+' + \\lambda_3$.<br>2. $\\lambda_3 = '+(var3-var1*2)+'$.</p>';
      } else {
        ex = '<p><strong>Advanced Problem ('+plainTopic+'):</strong> A Markov matrix $M$ satisfies $M^T M = '+var1+' I$. Is $M$ a pure isometric rotation?</p>'+
             '<p><strong>Step-by-Step Solution:</strong><br>1. Isometry requires $M^T M = I$.<br>2. Since '+var1+' \\ne 1$, $M$ scales lengths by $\\sqrt{'+var1+'}$ — a scaled rotation, not pure isometry.</p>';
      }
    } else if(t.s === 'ml'){
      ...
      if(scenario===0){
        ex = '<p><strong>Advanced Problem ('+plainTopic+'):</strong> A classifier uses step size $\\eta = 0.1$ with gradient $[-'+var1+', '+var2+']^T$. Find the weight update.</p>'+
             '<p><strong>Step-by-Step Solution:</strong><br>1. $w^{(t+1)} = w^{(t)} - \\eta \\nabla J(w)$.<br>2. $\\Delta w = -0.1 \\begin{pmatrix} -'+var1+' \\\\ '+var2+' \\end{pmatrix}$.<br>3. $= \\begin{pmatrix} '+(var1*0.1).toFixed(1)+' \\\\ -'+(var2*0.1).toFixed(1)+' \\end{pmatrix}$.</p>';
      } else if(scenario===1){
        ex = '<p><strong>Advanced Problem ('+plainTopic+'):</strong> An SVM uses polynomial kernel $K(x,y) = (x^T y + '+var1+')^{'+var2+'}$. With $x=[1,0]^T$, $y=[0,1]^T$, find $\\phi(x)^T \\phi(y)$.</p>'+
             '<p><strong>Step-by-Step Solution:</strong><br>1. Kernel trick: $\\phi(x)^T \\phi(y) = K(x,y)$.<br>2. $x^T y = 0$.<br>3. $K = '+var1+'^{'+var2+'} = '+Math.pow(var1,var2)+'$.</p>';
      } else {
        ex = '<p><strong>Advanced Problem ('+plainTopic+'):</strong> In k-means, point $P('+var1+', '+var2+')$ vs centroids $C_1(0,0)$ and $C_2('+var3+', 0)$. Verify the assignment.</p>'+
             '<p><strong>Step-by-Step Solution:</strong><br>1. $d_1^2 = '+(var1*var1)+' + '+(var2*var2)+' = '+(var1*var1+var2*var2)+'$.<br>2. $d_2^2 = '+Math.pow(var1-var3,2)+' + '+(var2*var2)+' = '+(Math.pow(var1-var3,2)+var2*var2)+'$.<br>3. Assign to the smaller; otherwise sub-optimal.</p>';
      }
    } else {
      ...
      if(scenario===0){
        ex = '<p><strong>Advanced Problem ('+plainTopic+'):</strong> Solve the recurrence $T(n) = '+var1+'T(n/'+var1+') + \\Theta(n)$.</p>'+
             '<p><strong>Step-by-Step Solution:</strong><br>1. Master theorem: $a='+var1+'$, $b='+var1+'$, $d=1$.<br>2. $\\log_b a = 1 = d$ → Case 2.<br>3. Tight bound: $\\Theta(n \\log n)$.</p>';
      } else if(scenario===1){
        ex = '<p><strong>Advanced Problem ('+plainTopic+'):</strong> Block nested loop join: table A $N='+var3+'$ pages, table B $M='+var1+'$ pages, '+var2+' buffer blocks. Minimum block transfers?</p>'+
             '<p><strong>Step-by-Step Solution:</strong><br>1. $T = N + \\lceil N/(B-2) \\rceil \\times M$.<br>2. $\\lceil '+var3+'/'+(var2-2)+' \\rceil = '+Math.ceil(var3/(var2-2))+'$.<br>3. Total: '+(var3 + Math.ceil(var3/(var2-2))*var1)+'.</p>';
      } else {
        ex = '<p><strong>Advanced Problem ('+plainTopic+'):</strong> Dijkstra with Fibonacci heap on $|V|='+var3+'$, $|E|='+(var1*var3)+'$. Dominant asymptotic term?</p>'+
             '<p><strong>Step-by-Step Solution:</strong><br>1. Bound: $O(|E| + |V| \\log |V|)$.<br>2. $|V| \\log |V|$ dominates the linear term.<br>3. Bottleneck: $O(|V| \\log |V|)$ from priority-queue extractions.</p>';
      }
    }
  }
