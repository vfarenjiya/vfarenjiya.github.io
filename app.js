...
function fmtShort(iso){ return new Date(iso+'T00:00:00').toLocaleDateString(undefined,{month:'short',day:'numeric'}); }
...
function loadState(){ try{ const res=localStorage.getItem('gate-quest-final-v1'); if(res) state={...state,...JSON.parse(res)}; isLightTheme=!!state.isLight; currentFontSize=state.fontScale||1; }catch(e){} }
function saveState(){ try{ localStorage.setItem('gate-quest-final-v1',JSON.stringify(state)); }catch(e){} }
...
function getMathematicalContent(t){
  ...
  if(MATH_DB[title]){
    ...
    if(t.s==='ps'){
      sub='<li>Moment Analysis</li><li>Asymptotic Convergence of '+plainTopic+'</li>';
...
function rowTemplate(t,groupKey){
  ...
  return '<li class="topic-item-container">'
    ...
    +'<div class="topic-main"><div class="topic-text">'+t.t+'</div>'
    +'<div class="topic-formula">'+(t.f?parseMathText('$'+t.f+'$'):parseMathText(t.tip||''))+'</div></div>'
    +'<button class="study-btn" onclick="toggleStudyPanel(\''+t.id+'\', event)">'+(isStudyOpen?'Close Module':'Study Math')+'</button>'
    +'</div><div class="study-panel '+(isStudyOpen?'open':'')+'">'+(isStudyOpen?getMathematicalContent(t):'')+'</div></li>';
}
...
$('resetBtn').addEventListener('click',()=>{
  if(confirm('Wipe ALL progress (topics, XP, coins, streaks)?')){ localStorage.removeItem('gate-quest-final-v1'); location.reload(); }
});
...
$('pwaDismiss').addEventListener('click',()=>{
  pwaBanner.classList.add('hidden'); state.pwaDismissed=true; saveState();
});
