'use strict';
/* ============ LUNAR LANDING MDP (tuned to be learnable) ============
   Units: meters, m/s, s. y = altitude up. Dynamics slowed (g=0.9) and
   limits widened so random exploration can occasionally land — otherwise
   the +100 reward is unreachable and Q-learning has nothing to bootstrap. */
const WORLD = { w: 100, h: 170, g: 0.9, dt: 0.2,
  padX0: 40, padX1: 60, softVy: 3.0, softVx: 2.5,
  aMain: 2.4, aLat: 1.0,
  rStep: -0.1, rCrash: -100, rWin: 100,
...
const phi = e => -(0.5 * e.y + 0.25 * Math.abs(e.x - 50)) / 100;

let Q = new Float64Array(STATES * 4);
let algo = 'qlearn';
const params = { alpha: 0.4, gamma: 0.99, epsStart: 1.0, epsEnd: 0.01, halfLife: 80 };
